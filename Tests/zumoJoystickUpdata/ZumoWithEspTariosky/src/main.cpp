...
const float conversion_factor =
    0.061;                    // [mg/LSB] Accelerometer sensitivity at +-2g
...
const unsigned long UpdateAverageInterval =
    1000;  // How often a value is added to the average
...
void updateAccelerationBuffer(float newXAcceleration, float newYAcceleration,
...
void displayWarnings(const AccelerationSums &averages,
                     const float *currentAccels) {
  ...
  if (averages.avgA > averageTreshold) {
    ...
    if (averages.avgX < -averageTreshold) {
      Serial.println(
          " You are generally breaking too hard, try slowing down before "
          "stops!");
    } else if (averages.avgX > averageTreshold) {
      Serial.println(
          "You are generally accelerating too fast! try a slower startup");
    }
  }
}
...
void updateAccelerationBuffer(float newXAcceleration, float newYAcceleration,
  ...
  bufferIndex =
      (bufferIndex + 1) % bufferlength;  // start på nytt dersom nådd 60
}
...
void receiveEvent(int howMany) {
  ...
  buzzer.playNote(NOTE_A(4), 1000, 2);
...
void updateAndSendI2CAccelData() {
  ...
  updateAccelerationBuffer(accelerations_m_s2[0], accelerations_m_s2[1],
                           accelerations_m_s2[2]);
  ...
  FloatUnion fu;
  fu.value = averages.avgA;  // Lagrer verdien til averages.avgA (gjennomsnitt
                             // total aks) som en union(delt opp i bytes)
